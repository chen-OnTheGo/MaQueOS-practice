...
#define TOP_BOUND 3
#define LEFT_BOUND 3
#define BOTTOM_BOUND 877
#define RIGHT_BOUND 1277
// 飞机坐标
int plane_x = 20;
int plane_y = 20;
struct plane_part {
...
void erase_plane(int x, int y)
{
	...
	for (i = 0; i < PLANE_NR; i++)
	{
		erase_char(x + plane_shape[i].dx, y + plane_shape[i].dy);
	}
}
...
void do_keyboard(unsigned char c)
{
    ...
    if (new_x < LEFT_BOUND || new_x > RIGHT_BOUND ||
        new_y < TOP_BOUND  || new_y > BOTTOM_BOUND)
    {
        return;  // 越界，不移动
    }

    // 3. 不越界：擦旧飞机
    erase_plane(plane_x, plane_y);
